...
export default function Sidebar() {
  ...
  // Get status icon for client
  const getStatusIcon = (status: string) => {
    switch (status) {
      case "active":
        return <CheckCircle className="h-3 w-3 text-green-600 flex-shrink-0" />
      case "pending":
        return <Clock className="h-3 w-3 text-yellow-600 flex-shrink-0" />
      case "inactive":
        return <PauseCircle className="h-3 w-3 text-gray-500 flex-shrink-0" />
      default:
        return <div className="h-3 w-3 rounded-full bg-gray-400 flex-shrink-0" />
    }
  }
                    ...
                    <div className={`space-y-3 pt-2 transition-all duration-300 delay-100 ${
                      ...
                      <div className="mt-3 border-t border-border pt-3">
                        <div className="bg-muted/30 rounded-lg border border-border p-2">
                          <div className="text-xs font-semibold text-card-foreground mb-2 px-1">
                            Clients
                          </div>
